...
def liste_proches(lat_depart, lon_depart, data, nombre, attribut):
    ...
    for index, row in data.iterrows():
        ...
        liste_distances = pd.concat([liste_distances, pd.DataFrame.from_dict({'index': index, 'lat' : lat_2, 'lon' : lon_2, 'distance' : distance_simple(lat_depart, lon_depart, lat_2, lon_2), 'nom' : nom})])
